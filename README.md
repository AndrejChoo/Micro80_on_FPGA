...
В качестве ОЗУ и ПЗУ используется микросхема SDRAM (достаточно любой микросхемы SDRAM с 8-битной шиной данных), в том числе реализована дополнительная ОЗУ на 48Кб. Прошивка монитора и CP/M хранятся в микросхеме SPI Flash 25-й серии, при запуске или после сброса данные из флешки переписываются в SDRAM и далее считываются уже оттуда. Также в память дополнителного ОЗУ записан интерпретатор BASIC (его можно запустить командой 
"G 0000" или просто "G0" из монитора, а перед использованием BASSIC необходимо ввести команду "NEW", иначе он не будет корректно работать). В предыдущей реализации вся память была реализована на микросхеме SRAM, такая возможность осталась, я в коде закоментировал всё, что касается SRAM. Но придётся подправить файл programmer.v, иначе автомат программирования зависнет в ожидании готовности SDRAM.
